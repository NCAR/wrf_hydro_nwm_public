...
/* 	printf("optarg = %s\n", optarg); */
	...
	if (strchr(optarg,':')) {
	  ...
	  if (bytend < bytnum) {
	    ...
	    fprintf(stderr, "\nFATAL ERROR: In gribbyte.c -  Exit *****\n\n");
	    exit(1);
	  }
	}
	else {
	...
	if (bytnum < 1) {
	  printf("FATAL ERROR: In gribbyte.c - Byte number must be greater than zero.\n");
	  exit (1);
	}
  ...
  if (fd == -1) {
    fprintf(stderr, "\nFATAL ERROR: Problem opening input file. *****\n");
  ...
  while (1) {
    ...
    case 2:
      ...
      break;
    default:
      fprintf(stderr,"FATAL ERROR: Edition number unsupported:  %i\n", edition);
      exit(1);
    }
    ...
    if (lengrib > MAXGRIB) {
      fprintf(stderr, "\nFATAL ERROR: ***** Increase MAXGRIB > %i\n", lengrib);
...
void process_grib1_record() {
  ...
  switch(sec1_ptr[7]){
  default:
    fprintf(stderr, "FATAL ERROR: Problem (1) with optional sections\n");
    exit (1);
  case 0:
  case 64:
    sec2_ptr = NULL;
  ...
  switch(sec1_ptr[7]){
  default:
    fprintf(stderr, "FATAL ERROR: Problem (2) with optional sections\n");
    exit (1);
  case 0:
  case 128:
    sec3_ptr = NULL;
...
void process_grib2_record (int lengrib) {
  ...
  while (ipt - ii < lengrib) {
    ...
    default:
      fprintf(stderr,"FATAL ERROR: This should not happen (3)");
      exit(1);
    case 1:
      sec1_ptr = &(ii[ipt-ii]);
...
void check_and_write() {
  ...
  case 0:
    ipt = ii;
    if (bytend > sec0_len) {
      fprintf(stderr, "FATAL ERROR: Byte request beyond section length\n");
      exit (1);
    }
    break;
  case 1:
    ipt = sec1_ptr;
    if (bytend > sec1_len) {
      fprintf(stderr, "FATAL ERROR: Byte request (%i) beyond section length (%i)\n", bytend, sec1_len);
      exit (1);
    }
    break;
  case 2:
  ...
  case 8:
    ...
    break;
  default:
    fprintf(stderr, "FATAL ERROR: Problem in specific byte:  Section %i\n", secval);
    exit (1);
  }
