...
*/
  ...
  if (strstr(filename,".bz2\0")) {
    ...
    if ( bzerror != BZ_OK ) {
      ...
      printf ("FATAL ERROR: In c_gribopen() - Problem BZopen.\n");
      exit (1);
    }
    lg->compression = 2;
  }
...
void c_close(struct GribFileInfo **GFPTR) {
  ...
  default:
    printf("FATAL ERROR: In c_close() - Unrecognized compression:  %i\n",(*GFPTR)->compression);
    exit (1);
#ifdef _BZIP_YES
  case 2:
    BZ2_bzReadClose ( &bzerror, (*GFPTR)->b );
    if ( bzerror != BZ_OK ) {
      printf ("FATAL ERROR: In c_close() - Problem with call to BZ2_bz_ReadClose.\n");
      exit (1);
    }
    fclose((*GFPTR)->fd);
    free((*GFPTR));
  ...
  case 0:
    ...
    if (ierr) {
      printf("FATAL ERROR: In c_close() - close error:  %i\n", ierr);
      exit (1);
    }
    free((*GFPTR));
    break;
...
void io_fread(struct GribFileInfo **GFPTR, char *buf, int *nread, int *iread, int *ierr) {
  ...
  switch ((*GFPTR)->compression) {
  default:
    printf("FATAL ERROR: In io_fread() - Unrecognized compression:  %i\n",(*GFPTR)->compression);
    exit (1);
#ifdef _BZIP_YES
  case 2:
...
void io_fseek(struct GribFileInfo **GFPTR, int *nbytes, int *mode) {
  int iseek;
  printf("FATAL ERROR: In io_fseek() - No fseek!\n");
  exit (1);
  switch (*mode) {
  case 0:
  ...
  case 2:
    ...
    break;
  default:
    printf("FATAL ERROR: In io_fseek() - Unrecognized mode:  %i\n", *mode);
    exit (1);
  }

  if (iseek != 0) {
    printf("FATAL ERROR: In io_fseek() - Seek problem.\n");
    exit (1);
  }
...
void io_ftell(struct GribFileInfo **GFPTR, long *itell) {
  *itell = ftell((*GFPTR)->fd);
  if (itell < 0) {
    printf("FATAL ERROR: In io_ftell() - Problem from ftell.\n");
    exit (1);
  }
}
