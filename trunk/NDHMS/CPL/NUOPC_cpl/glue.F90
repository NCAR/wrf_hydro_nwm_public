...
#define METHOD "NWM_NUOPC_Init"
  ...
  subroutine NWM_NUOPC_Init(did,vm,clock,itime,ntime,ntimestep,startdate,rc)
    ...
    integer, intent(out)        :: itime,ntime,ntimestep
    character(32), intent(out)  :: startdate
...
#define METHOD "NWM_NUOPC_Run"
  ...
  subroutine NWM_NUOPC_Run(did,mode,clock,itime, ntime, importState,exportState,rc)
...
#define METHOD "NWM_NUOPC_Fin"
  ...
  subroutine NWM_NUOPC_Fin(did,rc)
    ! ARGUMENTES
...
#ifdef WRF_HYDRO
    !call hydro_finish()  !?????? see above note
#endif
