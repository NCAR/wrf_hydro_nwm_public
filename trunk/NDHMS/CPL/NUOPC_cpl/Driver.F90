...
module Driver
...
#else
  use  FRONT_DUM, only: nwmSS => SetServices
#endif
#ifdef FRONT_MED
  ...
  type type_InternalStateStruct
    logical      :: verbose
    integer      :: modCount=0
    integer      :: med=0, atm=0, ocn=0, wav=0, ice=0, lnd=0, nwm=0
...
#define METHOD "SetServices" 
  ...
  subroutine SetServices(driver, rc)
    ...
    integer      ,pointer              :: med, atm, ocn, wav, ice, lnd, nwm
   ...
   ! query the component for its name
    ...
    ! *** report compiled modules, set model count, model index mapping, and
    ! model names ***
...
#define METHOD "SetModelServices"
  ...
  subroutine SetModelServices(driver, rc)
    ...
    do i = 1,modCount
      ...
      case ('NWM')
        call NUOPC_DriverAddComp(driver, modName(i), nwmSS, &
          petList=modPetList(i)%p, comp=modComp(i), rc=rc)
      case ('ATM')
  ...
  subroutine SetModelPetLists(driver, rc)
    ...
    call ESMF_ConfigGetAttribute(config, petLayoutOption, default='sequential', & label=trim(label), rc=rc)
    if (ESMF_LogFoundError(rc, PASSTHRU)) return ! bail out
    ...
    case ('sequential')
      ...
      if (ESMF_LogFoundError(rc, PASSTHRU)) then
        call ESMF_LogSetError(ESMF_FAILURE, rcToReturn=rc, &
          msg=trim(cname)//': '//trim(label)//' is required when pet_layout_option'// & ' = sequential')
        return ! bail out
      endif
      if (npet.lt.1.or.npet.gt.petCount) then
        call ESMF_LogSetError(ESMF_FAILURE, rcToReturn=rc, & msg=trim(cname)//': '//trim(label)//' must be > 0 and <= # PETs')
        return ! bail out
      endif
      do i = 1,modCount
        if (.not.modActive(i)) cycle
        ...
        if (ESMF_LogFoundAllocError(statusToCheck=stat, &
          msg='Allocation of '//modName(i)//' PET list array failed.', & CONTEXT, rcToReturn=rc)) return ! bail out
        do j = 1,npet
