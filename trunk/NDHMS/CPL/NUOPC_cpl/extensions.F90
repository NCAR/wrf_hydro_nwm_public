...
! Define ESMF real kind to match NWM single/double precision
#if defined(SINGLE)
...
!BOP
! !IROUTINE: NWM_Clock - Clock activities for NWM execution time
! !INTERFACE:
  ! 
  subroutine NWM_Clock(start_date, time_step, total_time, label,rc)
! ! ARGUMENTS
  character(len=*),intent(in),optional :: label, start_date
  integer, intent(out),optional        :: rc
  integer, intent(in), optional        :: time_step, total_time   ! TO DO
! !DESCRIPTION:
!   Write ESMF CplList to PET logs
!
!   The arguments are:
